...
export class ThemeSwitcherComponent implements AfterViewInit {
    ...
    public constructor(private renderer: Renderer2) {
        effect(() => {
            const htmlEl = this.renderer.selectRootElement('html', true);
            
            // Remove all previous Classes in <html> element
            this.renderer.removeClass(htmlEl, "dark");
            this.renderer.removeClass(htmlEl, "light");

            // Set darkMode and toggle Class
            this.renderer.addClass(htmlEl, this.darkMode() ? "dark" : "light");
            window.localStorage.setItem("darkMode", JSON.stringify(this.darkMode()))
        });
    }

    // Modify MatSlideToggleModule's Two Icons into Sun and Moon Based on Current Theme
    @ViewChild('darkModeToggle', { read: ElementRef }) themeToggleEl: ElementRef | undefined;
    ...
    ngAfterViewInit() {
        ...
        this.themeToggleEl.nativeElement.querySelector('.mdc-switch__icon--on').firstChild.setAttribute('d', this.moon);
        this.themeToggleEl.nativeElement.querySelector('.mdc-switch__icon--off').firstChild.setAttribute('d', this.sun);
        }
    }
