...
public class PasswordHasher : IPasswordHasher {
    ...
    public string HashPassword(string inputPassword) {
        ...
        var hash = Rfc2898DeriveBytes.Pbkdf2(inputPassword, salt, Iterations, _hashAlgorithmName, KeySize);
        
        return string.Join(Delimiter, Convert.ToBase64String(salt), Convert.ToBase64String(hash));
    }

    public bool Verify(string passwordHash, string inputPassword) {
        var splitHash = passwordHash.Split(Delimiter);
        var salt = Convert.FromBase64String(splitHash[0]);
        var hash = Convert.FromBase64String(splitHash[1]);
